...
|---|---|---|---|
| 1. Host a hive | host phone | `POST /rooms {code?}` → `{code, hostKey, joinUrl}`; Host · Lobby shows the code and a QR of `joinUrl` (`/j/<code>`); the host picks a track (`SET_TRACK`) | [06](06-hive-map-ui.md) |
...
| 4. Assignment | server | `plan(room)` gives each speaker `{label, role, color, gainsDb, delayMs, compensationMs, pattern, applyAtServerTime}` inside `ROOM_STATE` | [05](05-effect-modes.md) |
...
- **Target:** ≤10 ms device-to-device (`SYNC_TARGET_MS`; fusion with margin). **Floor:** ≤30 ms (`SYNC_FLOOR_MS`) — never ship worse; above it the room hears smear and we re-calibrate or cut.
...
| Output-latency unknown | 20–100+ (iOS unreliable; Android spread) | ±10–20 (spread within one browser family) | ±5 (limit of "sounds early / late" by ear) | ±2–3 (cross-correlation; ≤1 m to the host mic) |
...
| WAVE | ≤30 | intentional 0–300 ms delays and a 2 s swell dwarf the error | ok | ok | ok |
...
- The library ships **pre-separated demo tracks** under `fixtures/tracks/<id>/`: the synthetic `synthetic-60s` from `bun run fixtures` plus tracks we hold rights to ((assumption) CC-licensed or self-made; WAVs are gitignored, only `meta.json` is committed). The host gets a search box over that library (`GET /tracks?q=`).
...
- Uploading the host's own file (Replicate Demucs, `POST /tracks`) is B9 stretch only.
...
| HTTPS everywhere (Vercel + Fly) | Wake Lock, `getUserMedia` and `navigator.audioSession` are secure-context APIs |
| iOS ringer switch on, or `navigator.audioSession.type='playback'` (Safari 17+, set by the engine when available) | otherwise Web Audio is muted |
...
| Screens stay on | Wake Lock is requested inside `audio.unlock()`; audio dies with the tab on iOS |
| ≥6 phones for the finale, ≤64 (`MAX_PLAYERS`) | +7.8 dB at 6, +10 dB at 10; effects need bodies in the room |

## Canonical numbers

Every doc, brief and gate check must agree with `packages/protocol/src/constants.ts`. The identifiers below are the real ones.

| Constant | Value | Where it bites |
|---|---|---|
| `SYNC_TARGET_MS` / `SYNC_FLOOR_MS` | 10 / 30 | rig gates B3/B4; IC2 exit criterion |
...
| `NUDGE_RANGE_MS` | 100 | slider ±100 ms |
| `WAVE_SPAN_MAX_MS` / `WAVE_DEFAULT_SPAN_MS` / `WAVE_SWELL_PERIOD_MS` | 300 / 240 / 2000 | per-position delay and the travelling swell |
| `STROBE_DEFAULT_PERIOD_MS` / `STROBE_DEFAULT_DUTY` / groups default | 500 / 0.5 / 2 | strobe pattern |
| `STARTER_LATENCY_TABLE_MS` | ios-safari 60 · android-chrome 45 · desktop-chrome 25 · desktop-safari 30 · other null | starter guesses; B4 replaces them |
| `ROOM_CODE_LENGTH` / `MAX_PLAYERS` | 4 / 64 | codes from `ROOM_CODE_ALPHABET` (no 0/O/1/I/L) |
...
- **Frontend agent:** the pipeline table is the screen order; every join is a tap (audio unlock), and "Synced ±N ms" is `status.syncErrMs`. The tuning moment is a staged beat of the show, so Host · Calibrate deserves a countdown and progress rows, not a spinner.
...
| speaker | Any client with `plays:true` (players, and hosts that opted in); the planner covers speakers only |
| stem | One mono 16-bit WAV of a track (`drums`, `bass`, `vocals`, `other`, or `mix` for a non-stem track); ≤4 per track, ≤60 s |
| assignment | Per-client output of `plan(room)`: `{label, role, color, gainsDb, delayMs, compensationMs, pattern, applyAtServerTime}` |
| server time | Master clock, ms floats (`performance.timeOrigin + performance.now()` on the server); every scheduled event is expressed in it |
| compensation | `nudgeMs + (calibratedOffsetMs ?? tableLatencyMs ?? 0)`; positive = device is late → play it earlier |
