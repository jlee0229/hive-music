...
| A click's instant has already passed when the phone gets it (a late `SCHEDULED_ACTION`, a throttled tab) | the engine compares the mapped ctx time with `ctx.currentTime` | **the click is not played at all** (`/diag` counts it as `clicksSkipped`). Playing it late yields a residual wrong by exactly that lateness, which the server writes in as this phone's output latency; a missing measurement shows up as "not heard — run again", a late one is indistinguishable from a slow speaker. The tolerance is one render quantum, below which Web Audio cannot place it anyway |
