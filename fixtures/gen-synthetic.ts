/**
 * fixtures/gen-synthetic.ts — writes fixtures/tracks/<id>/{drums,bass,vocals,other}.wav + meta.json for
 * every synthetic track below. 16-bit PCM mono, deterministic (seeded LCG noise, reseeded per track so
 * generation order never matters). No npm deps. Run from the repo root: `bun run fixtures`.
 *
 * synthetic-60s:      44.1 kHz, 60.0 s, 120 BPM, drop at 30 s — the original demo track.
 * synthetic-30s-lite: 22.05 kHz, 30.0 s, 120 BPM, drop at 15 s — for docs/PROTOCOL-REQUESTS.md R-8:
 *   four 60 s stems at 44.1 kHz/16-bit mono are ~21 MB, so a 12-phone room is ~250 MB of parallel
 *   download before anything plays; halving the rate and the duration is ~5x less. The engine's
 *   mixed-sample-rate tests (packages/sync-client/src/__tests__/mixed-sample-rate.test.ts) already
 *   proved `decodeAudioData`/the scheduler are unaffected by file rate — this fixture just needs to
 *   exist with `durationSec` set from the file, not the sample count (same rule for every track here:
 *   DUR_SEC below already *is* what's written to durationSec, from the config, not derived from N).
 */
...
const TRACKS: TrackConfig[] = [
  { id: "synthetic-60s", title: "Synthetic 60", sampleRate: 44100, durationSec: 60, bpm: 120, dropSec: 30 },
  { id: "synthetic-30s-lite", title: "Synthetic 30 (lite)", sampleRate: 22050, durationSec: 30, bpm: 120, dropSec: 15 },
];

const TRACKS_DIR = join(dirname(fileURLToPath(import.meta.url)), "tracks");
const TWO_PI = 2 * Math.PI;

function generateTrack(cfg: TrackConfig): string[] {
  const { id, title, sampleRate: SR, durationSec: DUR_SEC, bpm: BPM, dropSec: DROP_SEC } = cfg;
  const N = SR * DUR_SEC;
  const BEAT_SEC = 60 / BPM;
  const OUT_DIR = join(TRACKS_DIR, id);

  // Seeded LCG (Numerical Recipes constants) → white noise in [-1, 1). Reseeded per track: same bytes every run.
  let seed = 0x5eed1234;
  ...
  // Beat grid (every BEAT_SEC) and drum-hit grid (beats, then eighth notes from the drop on).
  ...
  console.log(`${id}: energy (${energy.length} values): ${JSON.stringify(energy)}`);
  return written;
}

for (const cfg of TRACKS) generateTrack(cfg);
