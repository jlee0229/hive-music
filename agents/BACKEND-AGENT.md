...
| 3 | **B5e** | gain ramps from `assignment.gainsDb` (`setTargetAtTime`, 20 ms), `applyAtServerTime` honoured at the exact ctx time, pattern automation via `evaluatePattern` (100 ms look-ahead, `setValueCurveAtTime`), local mute | test: UNISON→ORCHESTRA changes gains only (no reload, no second `AUDIO_READY`); WAVE assignment produces a delayed start of `delayMs` | `evidence/backend/B5-engine.md` | ✅ 11 tests. UNISON→ORCHESTRA = 3 gains to 0 + 1 to unity, same source objects, one `AUDIO_READY` total; WAVE at spanMs 240 → starts 0/60/120/180/240 ms apart. **Design call:** WAVE is the one mode that is NOT a pure gain change — its `delayMs` moves the playhead, so a timing shift >`RESYNC_THRESHOLD_MS` reschedules (B4 turns that into a crossfade). Also fixed: identical 2 Hz snapshots were re-arming pattern automation into overlapping `setValueCurveAtTime` calls |
| 4 | **INT** | integration: PR to `main` ("engine v1"); with the frontend on the deployed stack, `NEXT_PUBLIC_HIVE_ENGINE=real` — **this is the first real phone test** | the human runs 3 phones in unison + orchestra + wave; you fix what they report | `evidence/backend/INT-first-test.md` | 🟡 **needs human** — PR [#2](https://github.com/jlee0229/hive-music/pull/2) open with 84 tests green. There are no phones in this environment, so the evidence file is the handover: 5 ordered checks (iOS silent switch first — it is the failure that looks healthy), the `ctx.outputLatency` measurements B4 needs to replace the guessed Tier-1 table, and the three known gaps so nobody debugs drift correction before B4 exists |
| 5 | **B4** | hard-resync drift handling with 20 ms crossfade; Tier-1 table measured per browser family you can reach; nudge end to end | simulated +50 ppm audio clock stays within 5 ms over 5 min; nudge +40 ms shifts the scheduled start by 40 ms exactly (unit) | `evidence/backend/B4-drift.md` | ✅ drift + 🟡 table. 9 tests. +50 ppm audio clock over 5 min → **mean 4.19 ms, worst 10.00 ms**, 1 resync (15 ms and climbing without the check); +50 ppm *system* clock → **1.45 ms** (the slew absorbs it). Nudge +40 ms moves the start 40 ms exactly. **The brief's "within 5 ms" is not reachable by a resync-at-10-ms design** — the sawtooth peaks at the threshold; the evidence file gives three ways to fix it and recommends promoting B9e (`playbackRate` slewing). Tier-1 table unchanged: measured Chromium here (32 ms interactive vs **72 ms playback** — confirms `latencyHint: "interactive"` matters) but this container has no sound card, so I shipped `rig/latency.html` for the human to fill the two rows that only exist on phones |
| 6 | **B8e** | `calibration.runAsReference` (mic with EC/AGC/NS off, worklet capture, cross-correlation, ±150 ms window, median anchoring, `CALIBRATION_REPORT`, mic release) + `renderClick`; **the measurement rig** `packages/sync-client/rig/` reusing the same DSP | unit: synthetic recording = template delayed 23.4 ms + noise → residual within **1 ms**; two templates 300 ms apart resolved independently; real run 🟡 for the human (+40 ms nudge recovered ±5 ms) | `evidence/backend/B8-calibration.md` | ✅ DSP + 🟡 real run. 13 tests. Click delayed 23.4 ms → **0.004 ms** (budget 1 ms); docs/04's 6-click case with one +7.3 ms and 37.2 ms of reference latency → **worst residual error 0.008 ms**; noise alone → confidence 0.079 (correctly ignored). `runAsReference` built (mic EC/AGC/NS off, Blob-URL worklet + ScriptProcessor fallback, graph-timestamped capture, mic release). **Rig validated against known skews** (`rig/synth-recording.ts`): 4/12/40 ms recovered to 0.01 ms — and the validation caught two bugs that would have produced fiction in a venue (truncated windows; sidelobes reported as extra phones, so two *synced* phones measured 1–8 ms apart). Real run + the +40 ms sign check are the human's, listed in the evidence file |
